...
- **Quotes**: Verbatim quotes that illustrate important points (anonymized)
...
*Note: Immediately anonymize any Personally Identifiable Information (PII) during this step.*
...
Use clear Markdown formatting with structured headers (H1, H2, H3). Each finding should stand on its own — a reader should be able to read any single finding and understand it without reading the rest.

Please utilize:
- **Bold text** for emphasis on key insights and finding statements.
- **Bullet points** for readability when listing evidence, quotes, or recommendations.
- **Tables** for Opportunity Sizing, Priority Matrices, or Persona summaries to make the data easy to scan and export to tools like Notion, Jira, or Confluence.
- **Blockquotes** (`>`) for verbatim user quotes.

## Tips

- **Protect User Privacy (PII)**: Always anonymize Personally Identifiable Information (PII). Replace real names, emails, and sensitive identifiers with contextual tags (e.g., "[Enterprise Admin]", "[User 1]").
- **Let the data speak**: Do not force findings into a predetermined narrative.
- **Distinguish behavior vs stated preference**: Behavioral data is stronger than what users simply say.
- **Use quotes effectively**: Include them generously as evidence, with attribution to participant type (not name).
- **State confidence levels**: A finding from 2 interviews is a hypothesis, not a conclusion.
- **Embrace contradictions**: Contradictions in the data are interesting; they often reveal distinct user segments.
- **Make actionable recommendations**: "Improve onboarding" is not actionable. "Add a progress indicator to the setup flow" is.
- **Prioritize relentlessly**: Resist the temptation to synthesize too many themes. 5-8 strong findings are better than 20 weak ones.